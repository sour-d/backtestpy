...
class LongShortSMAStrategy(BaseStrategy):
    """
    Long/Short SMA Strategy that can take both buy and sell positions.
    ...
    def __init__(self, env, fast_period=20, slow_period=50, risk_pct=0.02):
        """
        Initialize the Long/Short SMA strategy.
        
        Args:
            env: TradingEnvironment instance
            fast_period (int): Period for fast moving average
            slow_period (int): Period for slow moving average
            risk_pct (float): Risk percentage per trade (as decimal, e.g., 0.02 for 2%)
        """
        super().__init__(env)
    ...
    def liquidate_signal(self) -> dict:
        """
        Generate liquidate signal based on stop loss or opposite signal.
        ...
        # Check for opposite SMA signal (liquidate to prepare for opposite trade)
