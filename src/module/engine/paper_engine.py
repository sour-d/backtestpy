...
class PaperEngine:
    ...
    async def _initialize_components(self):
        ...
        if is_test_mode:
            ...
            try:
                initial_candles_df = pd.read_csv(
                    "data/test/raw/btcusdt_4h_all_2025.csv"
                )
                ...
                app_logger.error(
                    "Test data file not found at 'data/test/raw/btcusdt_4h_all_2025.csv'. Exiting."
                )
            ...
            initial_candles_raw = await temp_exchange.fetch_ohlcv(
        ...
        strategy = initialize_strategy(
            strategy_config, data_storage, portfolio, app_logger
        )
    ...
    async def run(self):
        ...
        app_logger.info(
            f"Starting paper trading for {strategy.data_storage.symbol} ({strategy.data_storage.timeframe})..."
        )

        if isinstance(data_storage, LiveDataManager):
            if data_storage.simulation_data is not None:
                # Simulation mode
                while True:
                    current_candle, historical_data = (
                        await data_storage.get_next_processed_data()
                    )
